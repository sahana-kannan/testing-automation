# iSERV Agentic Test Generation Pipeline (legacy)

> **This is the earlier, single-pass pipeline**, kept for reference and because
> it's the only implementation of the `customer_create_ticket`,
> `agent_create_ticket`, and `post_creation_visibility` flows. For new work,
> use the PRD-driven pipeline at the repo root (see the top-level README and
> `KT_fin.docx`) — it's the current, better-performing approach, currently
> implemented for `create_service_report`. All commands below are relative to
> this `legacy_ticket_flows/` directory.
...
Requirements and env file are shared at the repo root now (one `.env` covers
both pipelines — see the top-level `.env.example`).
...
All commands below assume your working directory is `legacy_ticket_flows/`
(`cd legacy_ticket_flows`) — the scripts load `../.env` automatically via
python-dotenv's upward search.
...
python record.py --record customer_create_ticket

# Step 2: run the entire pipeline
python pipeline.py --flow customer_create_ticket 
```
...
The raw recording is saved to `base_test_customer_create_ticket_recorded.py`.

### Step 2 — Build the parameterised base script

```powershell
python record.py --build customer_create_ticket
```

This reads the recorded file, replaces hardcoded values with `inputs.get(field)` calls, and writes the clean `base_test_customer_create_ticket.py`. You don't need to edit that file.
...
python extract.py --flow customer_create_ticket

# Stage 2: generate scenario matrix (calls OpenAI)
python generate.py --flow customer_create_ticket

# Stage 2b: re-apply sanitization to existing matrix without re-calling OpenAI
python apply_sanitize.py --flow customer_create_ticket
...
python execute.py --flow customer_create_ticket

# Stage 4 options:
python execute.py --flow customer_create_ticket --dry-run      # print plan, no browser
python execute.py --flow customer_create_ticket --id TC003     # run one scenario

# Stage 5: generate Excel report
python report.py --flow customer_create_ticket
```
...
```
python pipeline.py --flow <flow_name> [options]
...
python pipeline.py --flow customer_create_ticket --skip extract generate sanitize

# Re-run tests only, no regeneration
python pipeline.py --flow customer_create_ticket --only execute report

# Debug one failing test
python pipeline.py --flow customer_create_ticket --only execute --id TC007

# Check scenario routing before running
python pipeline.py --flow customer_create_ticket --only execute --dry-run
```
...
| File | Description |
|---|---|
| `context_{flow}.json` | Extracted source code context (Stage 1 output) |
| `scenario_matrix_{flow}.json` | Generated scenario matrix (Stage 2 output) |
| `base_test_{flow}_recorded.py` | Raw Playwright codegen output — do not edit |
| `base_test_{flow}.py` | Parameterised base script — do not edit |
| `storage_states/session_{role}.json` | Browser session state — gitignored |
| `raw_results_{flow}.json` | Per-scenario test results (Stage 4 output) |
| `report_{flow}.xlsx` | Excel report (Stage 5 output) |

---

## Adding a New Flow

1. **Create a manifest** at `manifests/{flow_id}.json` listing the source files to read (see existing manifests for the format).
